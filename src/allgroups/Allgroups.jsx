...
export default function Allgroups() {
  ...
  const getGroups = () => {
    axios
      .get("http://localhost:80/platform/react_project/groups.php")
  ...
  const AddToGroup = (groupId) => {
    ...
    axios
      .post(
        `http://localhost:80/platform/react_project/membersGroup.php/save`,
        inputs
      )
      .then((respone) => {
  ...
  const getPendingMempers = () => {
    axios
      .get(
        `http://localhost:80/platform/react_project/getPendingMember.php/${current_ID}`
      )
      .then((respone) => {
        console.log(respone.data);
  ...
  const getAcceptedMempers = () => {
    axios
      .get(
        `http://localhost:80/platform/react_project/getAcceptedMember.php/${current_ID}`
      )
      .then((respone) => {
        console.log(respone.data);
  ...
  const removeRequest = (GroupId) => {
    ...
    axios
      .put(
        `http://localhost:80/platform/react_project/getPendingMember.php/edit`,
        inputs
      )
      .then((respone) => {
        console.log(respone.data);
            ...
            <div className="groupList">
              <section className="blog">
                ...
                <h3> All Groups </h3>
                  ...
                  <div>
                    <Button variant="primary" onClick={() => ShowUpdateForm()}>
                      Create Group
                    </Button>
                  </div>
                      ...
                      return (
                        <div className="blogBox" key={index}>
                          <div className="blogImg">
                            <img
                              src={require(`../components/images/${ele.group_image}`)}
                              alt="Group1"
                            />
                          </div>
                          ...
                          <div className="blogText">
                            ...
                            <p className="blogDesc">{ele.group_description}</p>
                            <div className="blogBtn">
                              ...
                              {(() => {
                                if (
                                  pendingMembers.includes(ele.group_id) ||
                                  acceptedMembers.includes(ele.group_id)
                                ) {
                                  ...
                                  return (
                                    <Link>
                                      <button
                                        type="submit"
                                        className="blogBtn3"
                                        onClick={() => AddToGroup(ele.group_id)}
                                      >
                                        {" "}
                                        Add
                                      </button>

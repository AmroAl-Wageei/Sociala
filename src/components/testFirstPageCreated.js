...
function FirstPageCreated() {
  ...
  const [user , setUser] = useState([]);
    ...
    const likePost = async (e) => {
      e.preventDefault();
        await axios.post('http://localhost:80/platform/react_project/likes.php/' , inputs).then(
          getPosts()
        )
    }
  ...
  return (
      ...
      <div>
                  ...
                  {user.map((Oneuser , index_user)=>{
                   return(              
                    <img key={index_user} className="rounded-circle shadow-1-strong me-3" src={require(`./images/${Oneuser.image}`)} alt="avatar" style={{width : "5vw" , height : "5vw" , marginLeft : '14vw'}} />
                    )
                    })}
                    <form className="form-outline" onSubmit={handleImagePost}>
                      <textarea placeholder='Write something . . .' className="form-control"  id={current_ID} rows={4} style={{background: '#fff' , width : '50vw'}} onChange={handlePost}/>
                      <input type="file"
                      id="file"
                      ...
                      <button type="submit" className="btn btn-primary btn-sm" id="ShareBtnHome">Share</button>
                    </form>
                  </div>
            ...
            <div className="col-md-12 col-lg-10 col-xl-8">
              <div className="card">
                  ...
                  <div className="d-flex flex-start align-items-center">
                    <div>
                      <div style={{display : 'flex'}}>
                          <img className="rounded-circle shadow-1-strong me-3" src={require(`./images/${post.image}`)} alt="avatar" style={{width : "3vw" , height : "3vw"}} />
                        <div>
                          <h6 className="fw-bold text-primary mb-1">{post.first_name} {post.last_name}</h6>
                ...
                <form id={`editPostForm${post.post_id}`} action="" style={{display : 'none'}} onSubmit={handleEditPostSubmit}>
                  ...
                  <div className="small d-flex" style={{justifyContent : 'space-between' , gap : "1px"}}>
                    {
                    likes.map((like , index_like) => {
                      ...
                      })}<span className="postLikeCounter" style={{marginRight : '38vw' }}>({like_count})people like it</span>
                ...
                <div className="card-footer py-3 border-0" style={{backgroundColor: '#f8f9fa'}}>
                  ...
                  <div className="w-100" id={`littleCommentsOnPost${post.post_id}`}>
                  ...
                  { comments.map((comment,index_comment) => {
                    ...
                    {(comment_count > comment_count_show)?
                      <div id='showMoreComments' style={{cursor : 'pointer' , color : 'blue'}} onMouseOver={()=>{changeColor('showMoreComments')}} onMouseOut={()=>{returnColor('showMoreComments')}} onClick={()=>{showMoreComments(post.post_id)}}>Load more comments</div>
                      : null  
                    }
                  </div>
                  ...
                  <div className="w-100" id={`allCommentsOnPost${post.post_id}`} style={{display : 'none'}}>
                  <p style={{display : 'none'}}>{comment_count_show = 0}</p>
                  { comments.map((comment,index_comment) => {
                    if (comment.post_id == post.post_id){
                    return (
                    ...
                    {(comment_count > comment_count_show)?
                      <div id='showLittleComments' style={{cursor : 'pointer' , color : 'blue'}} onMouseOver={()=>{changeColor('showMoreComments')}} onMouseOut={()=>{returnColor('showMoreComments')}} onClick={()=>{showLittleComments(post.post_id)}}>Load less comments</div>
                      : null  
                    }
                  </div>
                  ...
                  {user.map((Oneuser , index)=>{
                   return(              
                    <img key={index} className="rounded-circle shadow-1-strong me-3" src={require(`./images/${Oneuser.image}`)} alt="avatar" style={{width : "3vw" , height : "3vw"}} />
                    )
                    })}
